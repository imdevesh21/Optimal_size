package solve;

public class wordSearch {

  private int rows;
  private int cols;
  private String targetWord;
  private char[][] gameBoard;

  public boolean exists(char[][]board, String word){
    rows = board.length;
    cols = board[0].length;
    targetWord = word;
    gameBoard = board;

    for(int i=0; i<rows; i++){
      for(int j=0; j<cols; j++){
        if(dfs(i, j, 0)) return true;
      }
    }
    return false;
  }

  public boolean dfs(int row, int col, int index){
    // if current cell does not matched target character
    if(gameBoard[row][col] != targetWord.charAt(index)){
      return false;
    }

    // if it's the last character and matched
    if(index == targetWord.length() - 1){
      return true;
    }

    // temporarily mark the current cell to be visited by replacing its value
    char tempChar = gameBoard[row][col];
    gameBoard[row][col] = '0';

    // define an array of directions {up, right, down, left};
    int[] directions = {-1, 0, 1, 0, -1};

    // Explore all possible adjcent cell
    for(int d=0; d<4; d++){
      int newRow = row + directions[d];
      int newCol = col + directions[d+1];

      // check if the position is withing bounds and not visited
      if(newRow >= 0 && newRow < rows && newCol >= 0 && newCol < cols && gameBoard[newRow][newCol] != '0'){
        if(dfs(newRow, newCol, index + 1)){
          return true;
        }
      }
    }
    gameBoard[row][col] = tempChar;
    return false;
  }
  public static void main(String[] args) {
    char[][] board = { { 'A', 'B', 'C', 'E' },
        { 'S', 'F', 'C', 'E' },
        { 'A', 'D', 'E', 'E' },
    };
    String[] testWords = { "SEE", "ABCB", "ABCCED", "ABCD", "ASFCCE" };

    wordSearch ws = new wordSearch();

    for (String word : testWords) {
        boolean result = ws.exists(board, word);
        System.out.printf("\"%-8s\" → %s\n", word, result);
    }
  }
}
